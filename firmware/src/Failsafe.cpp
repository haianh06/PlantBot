...
void sanityCheck(float temp, float humi, int soilMoistureRaw) {
  ...
  // Case 3: Bảo vệ đất quá ẩm (>95%)
  if (soilIsNormal && soilPercent > 95) {
  ...
  if (isSafeMode) {
    ...
    else if (currentErrorCode == SOIL_OVERWATER_ERROR && soilIsNormal &&
             soilPercent <= 80) {
      isSafeMode = false;
      currentErrorCode = NO_ERROR;
      pumpRelay.clearLock();
  ...
  if (now - lastHeartbeatTime >= 60000UL) {
    ...
    if (isAutoMode) {
      ...
      if (now - offlinePumpLastTime >= 21600000UL) {
        if (!pumpRelay.isOn() && !pumpRelay.isCooldown() &&
            !pumpRelay.isLocked()) {
          pumpRelay.turnOnWithTimeout(20000UL,
                                      300000UL); // Bơm 20s, 5m cooldown
          offlinePumpLastTime = now;
        }
      }
    }
...
void evaluateEnvironment(float temp, float humi, int soilPercent) {
  if (isSafeMode)
    return; // Nếu đã lỗi cảm biến thì ưu tiên Sanity Check
  ...
  if (soilPercent < 45) {
    if (!pumpRelay.isOn() && !pumpRelay.isCooldown() && !pumpRelay.isLocked()) {
      pumpRelay.turnOnWithTimeout(15000UL,
                                  300000UL); // 15s timeout, 5m cooldown
    }
  }
}
